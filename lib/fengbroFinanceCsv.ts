 ...
 * - imageUrls: url1;url2  (supports external https and Strapi 媒體庫 view URLs)
 * - relatedLinks: url  or  標籤|url  (multiple joined with `;`)
 *
 * imageUrls notes:
 * - Prefer full Strapi 媒體庫 URLs
...
/** Join image URLs for CSV (canonical Strapi 媒體庫 / https only). */
...
export function parseFinanceCustomCsv(text: string): {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    // Keep `;` / newlines; normalizeFinanceImageUrls understands both and Strapi 媒體庫 URLs.
