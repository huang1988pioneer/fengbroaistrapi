...
export async function fetchFilePartManifest(manifestUrl: string): Promise<FilePartManifest> {
  const response = await apiFetch(getProxiedMediaUrl(getAppwriteDownloadUrl(manifestUrl)), {
    headers: {
...
export async function resolveMultipartFileBlob(
  ...
  if (!isMultipartFiletype(source.filetype)) {
    const response = await apiFetch(getProxiedMediaUrl(getAppwriteDownloadUrl(fileUrl)));
    if (!response.ok) {
  ...
  for (const part of manifest.parts) {
    const response = await apiFetch(getProxiedMediaUrl(getAppwriteDownloadUrl(part.url)));
    if (!response.ok) {
